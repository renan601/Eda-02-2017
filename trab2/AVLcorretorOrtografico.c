...
Palavra* inicia_No(Palavra* a,char* valor){
    int i;
...
Palavra* insere_No(Palavra* l,char* palavra){
    Palavra* novo;

    if(l == NULL){
        l = inicia_No(l,palavra);
    }else{
        if(strcmp(palavra, l->valor) < 0){
            if(l->esq == NULL){
                novo = inicia_No(novo,palavra);
                l->esq = novo;
            }else{
                novo = insere_No(l->esq, palavra);
            }
        }else if(strcmp(palavra, l->valor) >= 0){
            if(l->dir == NULL){
                novo = inicia_No(novo,palavra);
                l->dir = novo;
            }else{
                novo = insere_No(l->dir,palavra);
            }
        }
    }
    return l;
